...
const addFakeFunctionsToGlobal = () => {
  ...
  extendedWindow.help = () => {
    ...
    console.log('%c  help()          - Show this help message', 'color: #96ceb4;');
    console.log('%c  wake.up()       - Attempt to wake the system (deprecated)', 'color: #96ceb4;');
    console.log('%c  coffee.inject() - Alternative wake method', 'color: #96ceb4;');
  ...
  extendedWindow.wake = {
    up: () => {
      console.error('%c❌ ERROR: Function wake.up() is deprecated since version 2.1.0', 'color: #ff6b6b; font-weight: bold;');
      console.warn('%c⚠️  Use coffee.inject() instead', 'color: #ffd93d;');
      console.log('%c📝 Migration guide: https://wake.fail/docs/migration (404)', 'color: #96ceb4;');
      return false;
    }
  };
  ...
  extendedWindow.coffee = {
    inject: () => {
      console.log('%c☕ Injecting coffee... Please wait...', 'color: #8B4513; font-weight: bold;');
      setTimeout(() => {
        console.log('%c✅ Coffee injection complete! System alertness: +15%', 'color: #4ecdc4;');
        console.log('%c🔋 Energy levels: [||||||||||||||||████] 80%', 'color: #96ceb4;');
      }, 2000);
  ...
  extendedWindow.systemStatus = () => {
    ...
    console.log('%c🖥️  System Status Report', 'color: #4ecdc4; font-size: 14px; font-weight: bold;');
    console.log('%c───────────────────────', 'color: #4ecdc4;');
    console.log(`%c   Status: ${randomStatus}`, 'color: #96ceb4;');
    console.log(`%c   Uptime: ${Math.floor(Math.random() * 999)}d ${Math.floor(Math.random() * 24)}h`, 'color: #96ceb4;');
    console.log(`%c   Load: ${(Math.random() * 3).toFixed(2)}`, 'color: #96ceb4;');
    console.log(`%c   Memory: ${Math.floor(Math.random() * 100)}% used`, 'color: #96ceb4;');
    console.log(`%c   Motivation: ${Math.floor(Math.random() * 50)}%`, 'color: #96ceb4;');
  ...
  extendedWindow.panic = () => {
    console.error('%c🚨 PANIC MODE ACTIVATED! 🚨', 'color: #ff6b6b; font-size: 16px; font-weight: bold; background: #000; padding: 8px;');
    console.error('%c🔥 EVERYTHING IS ON FIRE! 🔥', 'color: #ff6b6b; font-weight: bold;');
    console.warn('%c⚠️  Just kidding! Everything is fine... probably.', 'color: #ffd93d;');
    console.log('%c😌 Take a deep breath. It\'s just a loading screen.', 'color: #96ceb4;');
    displayAsciiArt('FAIL', '#ff6b6b');
    return '🧯 Fire extinguisher deployed.';
  };

  // Add a hint about the functions
  console.log('%c🎯 Fake functions loaded! Try them out:', 'color: #45b7d1; font-weight: bold;');
  console.log('%c   • help() - Your trusty guide', 'color: #96ceb4;');
  console.log('%c   • coffee.inject() - Essential for Monday mornings', 'color: #96ceb4;');
  console.log('%c   • systemStatus() - Check the chaos level', 'color: #96ceb4;');
};
