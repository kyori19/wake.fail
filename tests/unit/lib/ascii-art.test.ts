import { ASCII_ART, displayAsciiArt, displayRandomAsciiArt } from '../../../src/lib/ascii-art';
...
describe('ascii-art', () => {
  ...
  describe('ASCII_ART', () => {
    it('should contain expected ASCII art keys', () => {
      ...
      expectedKeys.forEach(key => {
        ...
        expect(ASCII_ART[key as keyof typeof ASCII_ART].length).toBeGreaterThan(0);
      });
    });
  ...
  describe('displayRandomAsciiArt', () => {
    ...
    it('should use one of the predefined colors', () => {
      const expectedColors = ['#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4', '#ffd93d'];
    ...
    it('should display one of the ASCII art pieces', () => {
      ...
      const hasExpectedArt = asciiValues.some(art => 
        firstArg.includes(art)
      );
