...
test.describe('wake.fail - Interactive Features', () => {
  ...
  test('cursor idle detection works', async ({ page }) => {
    ...
    const hasIdleIndicator =
      messageText?.includes('still there') ||
  ...
  test('window resize triggers layout effects', async ({ page }) => {
    ...
    const hasLayoutEffects =
      currentClasses?.includes('animate-pulse') ||
  ...
  test('favicon evolution works', async ({ page }) => {
    ...
    const faviconExists =
      (laterFavicon && laterFavicon.includes('data:')) ||
      laterFavicon?.includes('.ico');
  ...
  test('multiple clicks increase progress and update messages', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
